...
const AddChannelPartner = () => {
  ...
  useEffect(() => {
    ...
    } else if (citiesResult.isError) {
      toast.error(`Failed to fetch cities: ${citiesResult.error?.message || "Unknown error"}`);
    }
  }, [citiesResult.data, citiesResult.isError, citiesResult.error, dispatch]);

  const cityOptions = citiesResult?.data?.map((city: any) => ({
    value: city.value,
    text: city.label,
  })) || [];
  ...
  const handleChange = useCallback(
    ...
    [errors, formData.state, validateReraNumber]
  );
  ...
  const handleDropdownChange = useCallback(
    ...
    [errors, formData.reraNumber, validateReraNumber]
  );
  ...
  const validateForm = useCallback(() => {
    ...
    else if (formData.password.length > 12) newErrors.password = "Password should not exceed 12 characters";
  ...
  const handleSubmit = useCallback(
    ...
    [dispatch, formData, cityOptions, stateOptions, validateForm]
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50 to-white py-10 px-4">
      <div className="flex justify-end"><PageBreadcrumb items={[{ label: "Add Channel Partner" }]} /></div>
        ...
        >
          ...
          <div>
            ...
            <Input
              ...
              disabled={loading}
            />
            {errors.accountNumber && <p className="text-red-600 text-sm mt-1">⚠️ {errors.accountNumber}</p>}
          </div>
