...
const LeadForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    email: "",
    interestedProject: "",
    leadSource: isBuilder ? "" : "6", // Default to "6" for channel partners
    channelPartner: "",
    campaign: "",
    propertyType: "",
    squareFeet: "",
  ...
  useEffect(() => {
    if (isAuthenticated && user?.id) {
      if (isBuilder) {
        ...
        dispatch(fetchOngoingProjects({ admin_user_type: user.created_user_type, admin_user_id: user.created_user_id }));
      }
    }
    return () => {
      dispatch(clearUsers());
